...
class HybridRolePartitionedIDP:
    """
    ...
    def _idp_diversify(self, pop):
        """
        Immune Diversification Pulse (IDP).
        Perturbs a fraction of the population when diversity is low or stagnation persists.
        """
        frac = 0.2  # fraction of antibodies to perturb
        n_div = max(1, int(frac * len(pop)))

        # select weakest antibodies for diversification
        worst = sorted(pop, key=lambda ab: ab.affinity)[:n_div]

        for ab in worst:
            # Gaussian noise pulse around population mean
            mean_vec = np.mean([p.x for p in pop], axis=0)
            pulse = np.random.normal(loc=0.0, scale=0.1, size=self.dim)

            # update antibody position
            ab.x = mean_vec + pulse * (np.random.rand() * (self.bounds[:, 1] - self.bounds[:, 0]))
            
            # ensure bounds
            ab.x = np.clip(ab.x, self.bounds[:, 0], self.bounds[:, 1])

            # recompute affinity
            ab.affinity = -self.func(ab.x)
            self.eval_count += 1
    ...
    def minimize(self) -> Tuple[np.ndarray, float, Dict[str, Any]]:
        ...
        for gen in range(1, self.max_gens + 1):
            ...
            # update partition sizes (ensure consistency)
            ...
            explorers_selected = self._select_top(pop, self._explore_idx,
                                                max(1, int(self.n_select * (len(self._explore_idx)/self.N))))
            ...
            if (stagn >= self.stagn_thresh) or (entropy < self.entropy_frac_threshold):
                # Apply immune forgetting
                self._forget_in_place(pop)

                # 🔹 Conditional IDP injection
                self._idp_diversify(pop)

            # ---- periodic exchange ----
            if (gen % self.exchange_interval) == 0:
                explorers_sorted = sorted([(i, pop[i]) for i in self._explore_idx],
                                        key=lambda t: t[1].affinity, reverse=True)
                top_from_explore = [i for i, _ in explorers_sorted[: self.exchange_k]]
                if top_from_explore:
                    exploiters_sorted = sorted([(i, pop[i]) for i in self._exploit_idx],
                                            key=lambda t: t[1].affinity)
