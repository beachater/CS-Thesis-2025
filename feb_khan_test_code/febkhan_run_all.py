...
from hybrid_original_ibp_composed import HybridCSA_IBP_Composed
from hybrid_original_sodium import HybridCSAOriginal_Sodium
from reformed_hybrid import HybridRolePartitioned
from hybrid_reformed_idp import HybridRolePartitionedIDP
from hybrid_kill import HybridFCSA_BDT_OPF
...
def run_hybrid_ibp_hpip(obj, bounds, seed=None):
    opt = HybridCSA_IBP_Composed(obj, bounds, operator="ibp_hpip", seed=seed)
    x_best, f_best, info = opt.minimize()
    return np.array(info.get("history", [f_best]))

def run_hybrid_ibp_hp(obj, bounds, seed=None):
    opt = HybridCSA_IBP_Composed(obj, bounds, operator="ibp_hp", seed=seed)
    x_best, f_best, info = opt.minimize()
    return np.array(info.get("history", [f_best]))

def run_hybrid_ibp_ip(obj, bounds, seed=None):
    opt = HybridCSA_IBP_Composed(obj, bounds, operator="ibp_ip", seed=seed)
    x_best, f_best, info = opt.minimize()
    return np.array(info.get("history", [f_best]))

def run_hybrid_sodium(obj, bounds, seed=None):
    opt = HybridCSAOriginal_Sodium(obj, bounds, seed=seed)
    x_best, f_best, info = opt.minimize()
    return np.array(info.get("history", [f_best]))
...
def run_hybrid_reformed_idp(obj, bounds, seed=None):
    opt = HybridRolePartitionedIDP(obj, bounds, seed=seed)
    x_best, f_best, info = opt.minimize()
    return np.array(info.get("history", [f_best]))


def run_hybrid_bdt_opf(obj, bounds, seed=None):
    opt = HybridFCSA_BDT_OPF(obj, bounds, seed=seed)
    x_best, f_best, info = opt.minimize()
    return np.array(info.get("history", [f_best]))
...
algorithms = [
    # ("Hybrid Reformed IDp", run_hybrid_reformed_idp),
    ("IICO", run_iico),
    # # # # ("Hybrid IBP", run_hybrid_ibp),
    # # # # ("Hybrid IBP-HP", run_hybrid_ibp_hp),
    # # # # ("Hybrid IBP-IP", run_hybrid_ibp_ip),
    # # # # ("Hybrid IBP-HPIP", run_hybrid_ibp_hpip),
    # # # # ("Hybrid IBP-Sodium", run_hybrid_sodium),
    # ("Hybrid Reformed", run_hybrid_reformed),
    # ("Hybrid BDT-OPF", run_hybrid_bdt_opf),
    ("Hybrid sbm", run_hybrid_sbm),
    ("Hybrid DE", run_hybrid_original_de),
...
def run_all_dims():
    ...
    for dim in dims:
        ...
        fig_dir = os.path.join(os.path.dirname(__file__), f'../feb_khan_minitest_6_fig/dim_{dim}')
